    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.show_file_name && self.show_line_number {
            write!(
                f,
                "{}: {}: {}",
                self.filename, self.line_number, self.matched
            )
...
mod tests {
    ...
    fn printable_with_filename_and_linenumber() {
        let matched = MatchedLine {
            matched: true,
            line: String::from("testLine"),
        };
        let pwfn = PrintableWithFileNameLineNumber {
            line_number: 123,
        ...
        assert_eq!("fileName: 123: testLine", format!("{}", pwfn));
    }
    ...
    fn printable_linenumber() {
        ...
        assert_eq!("123: testLine", format!("{}", pwfn));
    }
    #[test]
    fn printable_filename() {
        ...
        assert_eq!("fileName: testLine", format!("{}", pwfn));
    }
    #[test]
    fn printable_line_only() {
        ...
        assert_eq!("testLine", format!("{}", pwfn));
    }
}
